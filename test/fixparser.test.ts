import { FIXParser, Fields, type Message } from '../src/FIXParser';
...
describe('FIXParser', () => {
    ...
    describe('#parse: ExecutionReport', () => {
        const messages: Message[] = fixParser.parse('8=FIX.4.4\x019=1753\x0135=8\x01523=S|E|B Auto-pricing\x01');
        let index = 0;
        expect(messages).toMatchSnapshot();
    ...
    describe('#getBriefDescription', () => {
        describe('should generate a brief description', () => {
            ...
            it('Messages with 58 field started from FIX prefix', () => {
                messages = fixParser.parse(
                    '8=FIX.4.4\x019=148\x0135=5\x0149=DEMO\x0156=FIX_LOCAL\x0134=3\x0152=20231107-18:23:21.983\x0158=FIX_LOCAL.Password is incorrect\x0110=067\x01',
                );
                expect(messages.length).toBe(1);
                expect(messages[0].data[7].tag).toBe(58);
                expect(messages[0].data[7].value).toBe('FIX_LOCAL.Password is incorrect');
                expect(messages[0].getBriefDescription()).toBe('Logout');
            });
        });
    });
