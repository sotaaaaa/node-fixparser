...
describe('FIXServer TCP', () => {
    ...
    it('End-to-end: connect and Logon - wait for heartbeat', (done) => {
                ...
                fixParser.connect({
                    ...
                    onMessage: (message: Message) => {
                        if (messageCounter === 0) {
                            ...
                            expect(fixServer.getNextTargetMsgSeqNum()).toEqual(2);
                            expect(fixServer.heartBeatInterval).toEqual(2);
                            expect(fixServer.fixVersion).toEqual('FIX.4.7');
                            messageCounter++;
                        } else if (messageCounter === 1) {
                            expect(message.description).toEqual('Heartbeat');
                            expect(fixServer.getNextTargetMsgSeqNum()).toEqual(3);
                            fixParser.close();
                            fixServer.destroy();
                            messageCounter++;
                            done();
